...
public class OptimizeArrayList<E> extends AbstractList<E> {
    ...
    public OptimizeArrayList(int capacity) {
        ...
        this.elements = (ArrayData<E>[]) new ArrayData[capacity];
    }

    @Override
    public void clear() {
        // 清空还是创建一块新的内存？
        this.elements = (ArrayData[]) new ArrayData[this.elements.length];
        size = 0;
        trimCapacityCheck();
    }
    ...
    public E get(int index) {
        rangeCheck(index);
        return elements[first + index].element;
    }

    @Override
    public E set(int index, E element) {
        rangeCheck(index);
        E oldElement = elements[first + index].element;
        elements[first + index].element = element;
        return oldElement;
    }
...
//        return oldElement;
    ...
    // 数组中存储的元素
    private static class ArrayData<E> {
        // 存储的内容
        E element;
        // 当前内容在数组中的索引
        int index;

        public ArrayData(E element, int index) {
